...
class HesBO:
    def __init__(self, f, lb, ub, low_dim, high_dim, n_init, max_evals):
    ...
    def optimize(self):
        best_results, elapsed, s, f_s, f_s_true, high_s = RunMain(low_dim=self.low_dim, high_dim=self.high_dim, initial_n=self.n_init, total_itr=self.max_evals, func_type='Branin', func=self.f,
            s=None, active_var=None, ARD=False, variance=1., length_scale=None, box_size=self.box_size, high_to_low=None, sign=None, hyper_opt_interval=20, noise_var=0)
        # print(best_results)
        # print(elapsed)
        self._X = s 
        self._fX = f_s
